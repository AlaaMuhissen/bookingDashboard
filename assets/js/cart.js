...
function getMyFlights() {
  ...
  return JSON.parse(flightsJSON) || carts;
}

function updateMyFlights(userFlight) {
      const flightsJSON = JSON.stringify(userFlight);
      localStorage.setItem('myFlights', flightsJSON); 
      const currentUserFlights=  getMyFlights();
      currentUserFlights.forEach(flight =>{
...
if(document.getElementById("book-button")){
  ...
  confirmButton.addEventListener("click", () => {
    ...
    carts.forEach((cart) =>{
      const index = flights.findIndex(flight => flight.id == cart.id);
      if(index != -1){
...
export { carts, addToCart, removeFromCart ,updateCartCount, getMyFlights,updateMyFlights};